...
├── Progress tracking
├── Error aggregation
├── Google Drive upload (upload_to_drive task)
└── Email report distribution (send_email_report task)
    └── Skipped for projects in SKIP_EMAIL_PROJECTS list
```

### run_all.py Integration Pattern

Projects can integrate automated Drive upload and email reporting:
...
# Define skip list for projects without XLSX state counts
SKIP_EMAIL_PROJECTS = ["audiobee_anthem", "audiobee_amerigroup", "audiobee_healthy_blue"]
...
TASKS = [
    ...
    "index_2.py",
    "index_3.py",
    "upload_to_drive",    # Upload 7z to Google Drive
    "send_email_report",  # Send state counts screenshot via email
]

# Handle upload_to_drive task
if task == "upload_to_drive":
...
# Handle send_email_report task
if task == "send_email_report":
    # Skip for projects without XLSX state counts
...
**SKIP_EMAIL_PROJECTS Pattern**: Projects that don't generate XLSX state counts (e.g., Anthem projects with custom output formats) should be added to the skip list to avoid FileNotFoundError during email report generation.
...
15. **Email Report Distribution**: Use `send_report_email()` from `tools/xlsx_to_clickup.py` in run_all.py for automated email distribution to operations@audiobee.ai - automatically skips projects in SKIP_EMAIL_PROJECTS list (Anthem projects without XLSX state counts)
16. **Credentials Security**: Never commit `tools/google_drive_credentials.json` or service account files - already excluded in .gitignore
